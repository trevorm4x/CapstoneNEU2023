...
class Tello:
    ...
    def change_vs_udp(self, udp_port):
        ...
        self.send_control_command(f'port 8889 {self.vs_udp_port}')
    ...
    def get_udp_video_address(self) -> str:
        ...
        print(f'Connecting to UDP at {address=}')
        return address
    ...
    def get_frame_read(self) -> 'BackgroundFrameRead':
        ...
        backgroundFrameRead.frame0 to get the actual frame0 received by the drone.
        Returns:
            BackgroundFrameRead
        """
...
class BackgroundFrameRead:
    ...
    backgroundFrameRead.frame0 to get the current frame0.
    """
    ...
    def __init__(self, tello, address):
        ...
        # Try grabbing a frame0 multiple times
        ...
        while time.time() - start < Tello.FRAME_GRAB_TIMEOUT:
            Tello.LOGGER.debug('trying to grab a frame0...')
        ...
        if not self.grabbed or self.frame is None:
            raise Exception('Failed to grab first frame0 from video stream')

        self.stopped = False
        self.worker = Thread(target=self.update_frame, args=(), daemon=True)

    def start(self):
        """Start the frame0 update worker
        Internal method, you normally wouldn't call this yourself.
        """
        self.worker.start()
